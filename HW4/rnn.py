...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        # dout: N, T, V
        dh, dW_vocab, db_vocab = temporal_fc_backward(dout, cache_out)
        ...
        dW_emb =  word_embedding_backward(dx, cache_emb)

        _, dW_proj, db_proj = fc_backward(dh0, cache_h0)
    ...
    def sample(self, features, max_length=30):
        ...
        current_index = self._start

        for t in range(max_length):
            word_embed, _ = word_embedding_forward(current_index, W_embed)     
            h, _ = rnn_step_forward(word_embed, prev_h, Wx, Wh, b)

            out, _ = fc_forward(h, W_vocab, b_vocab)
            current_index = np.argmax(out, axis=1)
            captions[:,t] = current_index
            prev_h= h
